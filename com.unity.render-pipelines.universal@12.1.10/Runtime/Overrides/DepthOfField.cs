...
namespace UnityEngine.Rendering.Universal
{
    ...
    public sealed class DepthOfField : VolumeComponent, IPostProcessComponent
    {
        ...
        public bool IsActive()
        {
            ...
            var isActive=  mode.value != DepthOfFieldMode.Gaussian || SystemInfo.supportedRenderTargetCount > 1;
           
            var hasValue = URPAdaptivePerformance.GetDOFEnable(isActive, out var result);
            if (hasValue && !result)
            {
                return false;
            }
