...
namespace UnityEditor.Rendering.Universal
{
    ...
    internal class ShaderPreprocessor : IPreprocessShaders
    {
        ...
        bool StripVolumeFeatures(VolumeFeatures features, Shader shader, ShaderSnippetData snippetData, ShaderCompilerData compilerData)
        {
            ...
            {
                if (shader.name == "Hidden/PostProcessing/TransitionSingleFocusSoft")
                {
                    return false;
                }
                var strings = ShaderBuildPreprocessor.GetCustomPostProcesssActiveShaderNames();

                if (!strings.Contains(shader.name))
                {
                    Debug.Log(" Remove  X-PostProcessing Shader Variant " + shader.name);
                    return true;
                }
            }
...
#endif
    {
        ...
        private static HashSet<string> s_CustomPostProcesssActiveShaderNames = new HashSet<string>();

        public static HashSet<string> GetCustomPostProcesssActiveShaderNames()
        {
            return s_CustomPostProcesssActiveShaderNames;
        }
        ...
        private static void FetchAllSupportedFeaturesFromVolumes()
        {
            ...
            s_CustomPostProcesssActiveShaderNames.Clear();
            ...
            foreach (var guid in guids)
            {
                ...
                var coms = asset.components;
                for (int i = 0; i < coms.Count; i++)
                {
                    var com = coms[i];
                    var sv = com as XPostProcessing.VolumeSetting;
                    if (sv!= null)
                    {
                        var shaderName = sv.GetShaderName();
                       // Debug.LogError("shader name"+ shaderName); 
                        if (!s_CustomPostProcesssActiveShaderNames.Contains(shaderName))
                        {
                            s_CustomPostProcesssActiveShaderNames.Add(shaderName);
                        }
                    }
                }
            }
